...
/**
 * print_version - Prints a version of ELF header.
...
/**
 * print_abi - Prints a ABI version of the ELF header.
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	reo = read(okr, header, sizeof(Elf64_Ehdr));
	if (reo == -1)
	{
		free(header);
		close_elf(okr);
